...
{
	// Guarda en la lista los contenidos del fichero
	
	if (FicheroListaCorrecto (nombre)) {
		
		// Reserva memoria para un elemento
		
		ReservarMemoria (1);	// LiberarMemoria con un puntero que no ha sido
								// definido da problemas (con basura)
		
		ifstream fichero (nombre);
		string cadena;
		
		// La primera línea únicamente contiene la cadena mágica
...
void Lista :: LeerLista (const string & nombre)
{
	// Lista temporal a partir del archivo
...
void Lista :: EscribirLista (const string & nombre) const
{
	// Abrir el fichero donde se guardará la lista
...
string Lista :: ToString (const string & const msg)
{
	string cad = "--------------------------------------------------\n";
	cad = cad + msg + "\n";
...
istream & operator >> (istream & entrada, Lista & lista)
{
	...
	while (entrada >> valor)	// Mientras no se llegue al final del fichero
		lista += valor;
...
bool Lista :: FicheroListaCorrecto (const string & nombre)
{
	ifstream fichero (nombre);
	bool correcto = true;	// También	correcto = fichero.fail();
	
	if (fichero.fail ())	// PREGUNTAR: ¿Cuál es la mejor forma de lidiar
		correcto = false;	//			  con estos errores?
							//			  ¿Sería adecuado usar un return varias
							//			  veces, o eso solo se hace en funciones
							//			  recursivas?
	
	if (correcto) {
		
		// Comprobar que la primera línea solo contenga la palabra LISTA
