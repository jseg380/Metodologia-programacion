...
// matriz a partir de los datos del fichero.
// Parámetros: nombre, fichero que contiene los datos para crear la matriz

Matriz2D :: Matriz2D (const string & nombre)
{
	// Guarda en la matriz los contenidos del fichero
	if (FicheroMatriz2DCorrecto (nombre)) {
		
		ifstream fichero (nombre);
		string cadena;
		
		// La primera línea solo contiene la cadena mágica
		
		ReservaMemoria (1, 1);	// Problemas al utilizar LiberaMemoria ()
								// con punteros cuyo contenido es basura (delete)
...
/***********************************************************************/
// Sustituye el contenido de la matriz por los valores
// que están en el fichero de texto nombre.
// Parámetros: nombre, fichero que contiene los datos para crear la lista

void Matriz2D :: LeerMatriz2D (const string & nombre)
{
	// Matriz temporal utilizando el constructor
...
// Parámetros: nombre, fichero donde se guardará la matriz
// NOTA: Si el fichero existe su contenido se
//		 se reemplazará por el de la matriz

void Matriz2D :: EscribirMatriz2D (const string & nombre) const
{
	// Abrir el fichero donde se guardará la matriz
	
	ofstream fichero (nombre);
	
	// Cadena mágica
	
	fichero << "MATRIZ2D" << endl;
...
istream & operator >> (istream & entrada, Matriz2D & matriz)
{
	// Se libera la memoria ocupada por matriz
	
	matriz.LiberaMemoria ();
	
	int filas, columnas;
	
	// Primera línea: filas y columnas respectivamente
...
ostream & operator << (ostream & salida, const Matriz2D & matriz)
{
	// Primera línea: filas y columnas con 6 casillas para cada número
	
	salida << setw(6) << matriz.fils << setw(6) << matriz.cols << endl;
	
	for (int f = 0 ; f < matriz.fils ; f++) {
		for (int c = 0 ; c < matriz.cols ; c++)
			salida << setw(ANCHO_MATRIZ) << matriz(f, c) << " ";
...
/***********************************************************************/
// Comprueba si un fichero de Matriz2D tiene el formato correcto.
// Parámetros: nombre, nombre del fichero

bool Matriz2D :: FicheroMatriz2DCorrecto (const string & nombre)
{
	ifstream fichero (nombre);
	bool correcto = true;
	...
	if (correcto) {
		...
		// Comprobar que la primera línea solo contenga la palabra MATRIZ2D
		...
		fichero >> cadena;	// Filas
		
		filas = stoi (cadena);
		
		fichero >> cadena;	// Columnas
		
		columnas = stoi (cadena);
		
		// Comprobar que aparece tantos números como filas x columnas
		
		while (fichero >> cadena)
			contador++;
		
		if ((filas * columnas) != contador)
			correcto = false;
	}
