...
//
// Definición de la clase Secuencia (versión 3).
// Incluye: 
...
Secuencia :: Secuencia (int la_capacidad)
	: capacidad (la_capacidad), total_utilizados (0) 
{
...
Secuencia :: Secuencia (const char * nombre) 
	: capacidad (TAMANIO), total_utilizados (0) 
{
...
// Versión 2:  [Secuencia] + [TipoBaseSecuencia]
//
...
//	tmp.AniadeValor (valor);
	...
	return (sec+tmp); // Version 1: [Secuencia] + [Secuencia]
...
// Versión 3:  [TipoBaseSecuencia] + [Secuencia] 
//
...
//	tmp.Inserta (0, valor); // Inserta en la casilla 0 

	return (tmp + sec); // Version 1: [Secuencia] + [Secuencia]
...
string Secuencia :: ToString(const char * const msg) const
{
    string cadena;

	if (strlen(msg)!=0) cadena = msg;
...
/***********************************************************************/
/***********************************************************************/
// Operator << Inserta en un flujo de texto un objeto "Secuencia".

ostream & operator << (ostream & out, const Secuencia & s)
{
	out.setf (ios::fixed);
    ...
    for (int i=0; i<s.total_utilizados; i++) {
		...
		TipoBaseSecuencia valor = s.vector_privado[i];

		#ifdef INT
		out << setw(ANCHO_SECUENCIA) << valor;	
		#else
		#ifdef CHAR
		out << setw(ANCHO_SECUENCIA) << valor;	
		#else
		#ifdef DOUBLE
		out << setw(ANCHO_SECUENCIA) 
            << setprecision(DECIMALES_SECUENCIA) << valor;	
		#endif
		#endif
		#endif
...
istream & operator >> (istream & in, Secuencia & s)
{	
	// Limpiar el objeto: Liberar memoria reservada previamente
	s.LiberarMemoria();
	
	TipoBaseSecuencia dato_leido;
...
/***********************************************************************/
/***********************************************************************/
// Método que guarda el contenido de una "Secuencia" en un fichero de 
// texto llamado "nombre".

void Secuencia:: EscribirSecuencia (const char * nombre) const
{
	ofstream fo (nombre);
...
/***********************************************************************/
/***********************************************************************/
// Método que rellena una Secuencia (que ya existe) a partir de
...
void Secuencia :: LeerSecuencia (const char * nombre)
{
	// Limpiar el objeto: Liberar memoria reservada previamente
	LiberarMemoria ();
...
/***************************************************************************/
/***************************************************************************/
// Rellena un dato "Secuencia" con los datos guardados en el fichero de  
// texto llamado "nombre".
//
...
//	void Secuencia :: LeerSecuencia (const char * nombre);
// 
// PRE: La Secuencia está vacía

void Secuencia :: RecuperaDatosDeFichero (const char * nombre)
{
	ifstream fi (nombre);
	if (!fi) {
	...
	if (!strcmp(cadena, "SECUENCIA")) {	// CORRECTO --> Seguir leyendo

		fi >> (*this);
	}
	else {
		cerr << "Error en fichero " << nombre << ": ";
		cerr << "No es un fichero SECUENCIA" << endl; 
	}
