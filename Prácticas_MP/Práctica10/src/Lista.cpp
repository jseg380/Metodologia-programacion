...
bool Lista :: operator  > (const Lista & otra)
{
	...
	TipoNodo * ptr_este = primero;
	TipoNodo * ptr_otra = otra.primero;
	
	// Calcula la longitud mínima de las cadenas
	
	int longitud;
	
	if (Tamanio() > otra.Tamanio())
		longitud = otra.Tamanio();
	else
		longitud = Tamanio();
	
	// Se compara valor a valor según la posición. Si en una posición
	// el valor de la lista implícita es mayor que el de la explícita,
	// entonces la lista implícita "es mayor" que la explícita
	
	for (int i = 1 ; i <= longitud && !mayor ; i++) {
		if ((*this)[i] > otra[i])
			mayor = true;
		// Si ha llegado al final de la lista más pequeña sin encontrar cuál
		// es mayor entonces la lista más larga será la mayor
		
		else if (i == longitud && Tamanio() > otra.Tamanio())
			mayor = true;
	}
